...
static void xlate_actions__(struct xlate_in *, struct xlate_out *)
	...
	static void xlate_fp(struct xlate_ctx *ctx, uint8_t bitmap, bool may_packet_in);
	...
	static void execute_fp_actions(struct xlate_ctx *ctx, struct AT_MATCH_ENTRY *at_entry);
...
static void xlate_fp(struct xlate_ctx *ctx, uint8_t bitmap, bool may_packet_in)
{
	...
	/* 
	*	Assume at most 8 apps, id from 0~7
	*	bitmap:	1	1	1	1	1	1	1	1 
	*	appid:	7	6	5	4	3	2	1	0
	*/
	for(i = 0; i < 8; i ++)
	{
		if(!(bitmap & 1 << i))
...
//        else
		...
		HMAP_FOR_EACH_WITH_HASH(at_entry, node, hash_string(at_match_string, 0), &(at->at_entrys))
		{
			if(at_entry->last_status == next_state)
			{
				...
				execute_fp_actions(ctx, at_entry);
                break;
			}
		}
