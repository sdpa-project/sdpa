...
import org.sdpa.protocol.*;
import org.sdpa.protocol.SDPAAction.ActType;


/**
 * Stateful firewall implemented based on sdpa.
...
//			//logger.info("isInit");
//			//doSendSdpaInitMsg(sw);
//			//this.isInit = true;
//		}
			
	}
			
	public void doSendSdpaInitMsg(IOFSwitch sw){
			
		logger.info("-----send init msg to {}, type is {}------",sw.getId(),OFType.SDPA_CREATE.getTypeValue());
		//long bitMap = 109150208;
		//init st table
		SDPASt st_tmp = new SDPASt();
		st_tmp.setAppid(appId);
		st_tmp.setBitmap(bitMap);
			
			
		//init at table
		SDPAAt at_tmp = new SDPAAt();
		at_tmp.setBitmap(bitMap);
		//SDPAAction act = new SDPAAction();
		//act.setAction(ActType.ACT_OUTPUT, 1);
		//ATDATA atdat = new ATDATA(1,"122.111.23.3".getBytes(),act);
		//at_tmp.addAtData(atdat);
				
		//init stt table
		SDPAStt stt_tmp = new SDPAStt();
		//STTDATA sttdat = new STTDATA(SDPAEventType.SDPAPARAM_IN_PORT,1,
		//SDPAEventType.SDPAPARAM_DL_SRC,2,SDPAEventOp.OPRATOR_ISEQUAL,1,2);
		STTDATA []sttdat = {new STTDATA(SDPAEventType.SDPAPARAM_TP_FLAG, 1, SDPAEventType.SDPAPARAM_CONST, 2, SDPAEventOp.OPRATOR_ISEQUAL,
				StateFirewallStatus.SFW_STATUS_REQUESTER_NONE.getValue(), StateFirewallStatus.SFW_STATUS_REQUESTER_SYN_SENT.getValue()),
							new STTDATA(SDPAEventType.SDPAPARAM_TP_FLAG, 1, SDPAEventType.SDPAPARAM_CONST, 2, SDPAEventOp.OPRATOR_ISEQUAL,
		...
		SDPACreate sfc = (SDPACreate) floodlightProvider.getOFMessageFactory().getMessage(OFType.SDPA_CREATE);
...
//		        }
	        ...
	        //s    .setInstructions(Arrays.asList((OFInstruction) new OFInstructionApplyActions().setActions(actions)));
		        ...
		        String strFwd = SDPAUtil.getFDirectionDataFromPktIn(bitMap, pi, cntx);
		        System.out.println(strFwd);
		        String strBwd = SDPAUtil.getBDirectionDataFromPktIn(bitMap, pi, cntx);
		        ...
		        SDPAMod stmodmsg = new SDPAMod();
		        stmodmsg.addMsg(appId, SDPAModType.ENTRY_ADD, stdat1);
		        stmodmsg.addMsg(appId,SDPAModType.ENTRY_ADD , stdat2);
		        
		        stmodmsg.sendmsg(sw);
		        
		        //step 2.3 make at entrys
		        SDPAMod atmodmsg = new SDPAMod();
		        ATDATA atdat1 = new ATDATA(StateFirewallStatus.SFW_STATUS_REQUESTER_NONE.getValue(),strFwd.getBytes(),new SDPAAction(ActType.ACT_OUTPUT,outport));
		        ATDATA atdat2 = new ATDATA(StateFirewallStatus.SFW_STATUS_RESPONSER_NONE.getValue(),strBwd.getBytes(),new SDPAAction(ActType.ACT_OUTPUT,routport));
		       
		        ATDATA atdat3 = new ATDATA(StateFirewallStatus.SFW_STATUS_REQUESTER_SYN_SENT.getValue(),strFwd.getBytes(),new SDPAAction(ActType.ACT_OUTPUT,outport));
		        ATDATA atdat4 = new ATDATA(StateFirewallStatus.SFW_STATUS_RESPONSER_ESTABLISH.getValue(),strBwd.getBytes(),new SDPAAction(ActType.ACT_OUTPUT,routport));
		        ATDATA atdat5 = new ATDATA(StateFirewallStatus.SFW_STATUS_REQUESTER_ESTABLISH.getValue(),strFwd.getBytes(),new SDPAAction(ActType.ACT_OUTPUT,outport));
		        ATDATA atdat6 = new ATDATA(StateFirewallStatus.SFW_STATUS_REQUESTER_FIN_SENT.getValue(),strFwd.getBytes(),new SDPAAction(ActType.ACT_OUTPUT,outport));
		        ATDATA atdat7 = new ATDATA(StateFirewallStatus.SFW_STATUS_DEFAULT_ERROR.getValue(),strFwd.getBytes(),new SDPAAction(ActType.ACT_DROP,0));
		        ATDATA atdat8 = new ATDATA(StateFirewallStatus.SFW_STATUS_DEFAULT_ERROR.getValue(),strBwd.getBytes(),new SDPAAction(ActType.ACT_DROP,0));
		        
		        atmodmsg.addMsg(appId, SDPAModType.ENTRY_ADD, atdat1);
		        atmodmsg.addMsg(appId, SDPAModType.ENTRY_ADD, atdat2);
		        atmodmsg.addMsg(appId, SDPAModType.ENTRY_ADD, atdat3);
		        atmodmsg.addMsg(appId, SDPAModType.ENTRY_ADD, atdat4);
		        atmodmsg.addMsg(appId, SDPAModType.ENTRY_ADD, atdat5);
		        atmodmsg.addMsg(appId, SDPAModType.ENTRY_ADD, atdat6);
		        atmodmsg.addMsg(appId, SDPAModType.ENTRY_ADD, atdat7);
		        atmodmsg.addMsg(appId, SDPAModType.ENTRY_ADD, atdat8);
