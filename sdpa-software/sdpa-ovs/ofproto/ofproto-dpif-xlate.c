...
static int xlate_fp(struct xlate_ctx *ctx, uint8_t bitmap, bool may_packet_in);
static uint64_t get_event_param(struct xlate_ctx *ctx, enum PARAM_TYPE param);
static bool get_event(uint64_t param1, uint64_t param2, enum OPRATOR op);
static int  execute_fp_actions(struct xlate_ctx *ctx, struct AT_MATCH_ENTRY *at_entry);
...
*/
static char*  parse_match(struct xlate_ctx *ctx, uint64_t bitmap)
{
	char* match = NULL;
	...
	for(i = 0; i < 26; i ++)
	{
		if(bitmap & 1 << i)
		{
			char *temp = xmalloc(1024);
            int n;
			switch(i)
			{
				case 1:  // ovs_be64 metadata
				{
					n = sprintf(temp , "%"PRIu64"", ctx->xin->flow.metadata);
					break;
				}
				case 5:  // union flow_in_port in_port
				{
					n = sprintf(temp, "%"PRIu32"", ctx->xin->flow.in_port.ofp_port);
					break;
				}
				case 6:  // uint8_t dl_src[6]
				{
					char* temptemp;
					int k = 0;
					n = sprintf(temp, "%PRIu8", ctx->xin->flow.dl_src[0]);
					for(k = 1; k < 6; k ++)
					{
						n = sprintf(temptemp , "%"PRIu8"", ctx->xin->flow.dl_src[k]);
						strcat(temp, temptemp);
					}
					break;
				}
				case 7:  // uint8_t dl_dst[6]
				{
					char* temptemp;
					int k = 0;
					n = sprintf(temp, "%PRIu8", ctx->xin->flow.dl_dst[0]);
					for(k = 1; k < 6; k ++)
					{
						n = sprintf(temptemp , "%"PRIu8"", ctx->xin->flow.dl_dst[k]);
						strcat(temp, temptemp);
					}
					break;
				}
				case 8:  // ovs_be16 dl_type
				{
					n = sprintf(temp , "%"PRIu16"", ctx->xin->flow.dl_type);
					break;
				}
				case 9:  // ovs_be16 vlan_tci
				{
					n = sprintf(temp , "%"PRIu16"", ctx->xin->flow.vlan_tci);
					break;
				}
				case 10: // ovs_be32 mpls_lse
				{
					n = sprintf(temp , "%"PRIu32"", ctx->xin->flow.mpls_lse);
					break;
				}
				/*case 11: // struct in6_addr ipv6_src
				{
					char* temptemp;
					int k = 0;
					n = sprintf(temp, "%PRIu8", ctx->xin->flow.ipv6_src.u.u_s6_addr[0]);
					for(k = 1; k < 16; k ++)
					{
						n = sprintf(temp, "%PRIu8", ctx->xin->flow.ipv6_src.u.u_s6_addr[k]);
						strcat(temp, temptemp);
					}
					break;
				}
				case 12: // struct in6_addr ipv6_dst
				{
					char* temptemp;
					int k = 0;
					n = sprintf(temp, "%PRIu8", ctx->xin->flow.ipv6_dst.u.u_s6_addr[0]);
					for(k = 1; k < 16; k ++)
					{
						n = sprintf(temp, "%PRIu8", ctx->xin->flow.ipv6_dst.u.u_s6_addr[k]);
						strcat(temp, temptemp);
					}
					break;
				}
				case 13: // struct in6_addr nd_target
				{
					char* temptemp;
					int k = 0;
					n = sprintf(temp, "%PRIu8", ctx->xin->flow.nd_target.u.u_s6_addr[0]);
					for(k = 1; k < 16; k ++)
					{
						n = sprintf(temp, "%PRIu8", ctx->xin->flow.nd_target.u.u_s6_addr[k]);
						strcat(temp, temptemp);
					}
					break;
				}*/
				case 14: // ovs_be32 ipv6_label
				{
					n = sprintf(temp , "%"PRIu32"", ctx->xin->flow.ipv6_label);
					break;
				}
				case 15: // ovs_be32 nw_src
				{
                    struct in_addr addr1;
                    memcpy(&addr1, &(ctx->xin->flow.nw_src), 4);                    
                    strcpy(temp, inet_ntoa(addr1));
					break;
				}
				case 16: // ovs_be32 nw_dst
				{
					struct in_addr addr2;
                    memcpy(&addr2, &(ctx->xin->flow.nw_dst), 4);                    
                    strcpy(temp, inet_ntoa(addr2));
					break;
				}
				case 17: // uint8_t nw_frag
				{
					n = sprintf(temp , "%"PRIu8"", ctx->xin->flow.nw_frag);
					break;
				}
				case 18: // uint8_t nw_tos
				{
					n = sprintf(temp , "%"PRIu8"", ctx->xin->flow.nw_tos);
					break;
				}
				case 19: // uint8_t nw_ttl
				{
					n = sprintf(temp , "%"PRIu8"", ctx->xin->flow.nw_ttl);
					break;
				}
				case 20: // uint8_t nw_proto
				{
					n = sprintf(temp , "%"PRIu8"", ctx->xin->flow.nw_proto);
					break;
				}
				case 21: // uint8_t arp_sha[6]
				{
					char* temptemp;
					int k = 0;
					n = sprintf(temp, "%PRIu8", ctx->xin->flow.arp_sha[0]);
					for(k = 1; k < 6; k ++)
					{
						n = sprintf(temptemp , "%"PRIu8"", ctx->xin->flow.arp_sha[k]);
						strcat(temp, temptemp);
					}
					break;
				}
				case 22: // uint8_t arp_tha[6]
				{
					char* temptemp;
					int k = 0;
					n = sprintf(temp, "%PRIu8", ctx->xin->flow.arp_tha[0]);
					for(k = 1; k < 6; k ++)
					{
						n = sprintf(temptemp , "%"PRIu8"", ctx->xin->flow.arp_tha[k]);
						strcat(temp, temptemp);
					}
					break;
				}
				case 23: // ovs_be16 tcp_flags
				{
					n = sprintf(temp , "%"PRIu16"", ctx->xin->flow.tcp_flags);
					break;
				}
				case 25: // ovs_be16 tp_src
				{
					n = sprintf(temp , "%"PRIu16"", ctx->xin->flow.tp_src);
					break;
				}
				case 26: // ovs_be16 tp_dst
				{
					n = sprintf(temp , "%"PRIu16"", ctx->xin->flow.tp_dst);
					break;
				}
				default: // haven't defined
				{
					printf("**Match field ID not defined: id=%d\n", i);
					break;
				}
			}
			if(!match)  match = temp;
			else        strcat(match, temp);
		}
	}
//	printf("match is: %s\n", match);
	return match;
...
// get event param according to param type
static uint64_t get_event_param(struct xlate_ctx *ctx, enum PARAM_TYPE param)
{
	uint64_t value;
	switch(param)
	{
		case SFAPARAM_IN_PORT: /* Switch input port. */
		{
			value = ctx->xin->flow.in_port.ofp_port;
			break;
		}
		case SFAPARAM_DL_VLAN: /* VLAN id. */
		{
			value = ctx->xin->flow.vlan_tci;
			break;
		}
		case SFAPARAM_DL_VLAN_PCP: /* VLAN priority. */
		{
			value = -1;
			break;
		}
		case SFAPARAM_DL_TYPE: /* Ethernet frame type. */
		{
			value = ctx->xin->flow.dl_type;
			break;
		}
		case SFAPARAM_NW_TOS: /* IP ToS (DSCP field, 6 bits). */
		{
			value = ctx->xin->flow.nw_tos;
			break;
		}
		case SFAPARAM_NW_PROTO: /* IP protocol. */
		{
			value = ctx->xin->flow.nw_proto;
			break;
		}
		case SFAPARAM_TP_SRC: /* TCP/UDP/SCTP source port. */
		{
			value = ctx->xin->flow.tp_src;
			break;
		}
		case SFAPARAM_TP_DST: /* TCP/UDP/SCTP destination port. */
		{
			value = ctx->xin->flow.tp_dst;
			break;
		}
		case SFAPARAM_TP_FLAG: /* TCP FLAG  */
		{
			value = ctx->xin->tcp_flags;
            //printf("Param type: TCP FLAG, value is: %lld\n", value);
			break;
		}
		case SFAPARAM_DL_SRC: /* Ethernet source address. */
		{
			value = -1;
			break;
		}
		case SFAPARAM_DL_DST: /* Ethernet destination address. */
		{
			value = -1;
			break;
		}
		case SFAPARAM_NW_SRC: /* IP source address. */
		{
			value = ctx->xin->flow.nw_src;
			break;
		}
		case SFAPARAM_NW_DST: /* IP destination address. */
		{
			value = ctx->xin->flow.nw_dst;
			break;
		}
		case SFAPARAM_METADATA: /* Upper level data	*/
		{
			value = ctx->xin->flow.metadata;
			break;
		}
		default: 
		{
...
static int execute_fp_actions(struct xlate_ctx *ctx, struct AT_MATCH_ENTRY *at_entry)
{
	...
	switch(at_entry->act.actype)
	{
		case SAT_OUTPUT:
		{
            ...
            printf("  Action type is Output, however in our new design this action shouldn't exist.\n");
			break;
		}
		case SAT_DROP:
		{
...
static int xlate_fp(struct xlate_ctx *ctx, uint8_t bitmap, bool may_packet_in)
{
	printf("--Inside function xlate_fp!\n");
	...
	if(!app_found)
	{
		printf("**No relavant id APP found!\n");
		return;
		//break;
	}
	...
	char* at_match_string = parse_match(ctx, at_match_bitmap);
	if(!st_match_string)
    {
        printf("**Match field NULL!\n");
        return;
	...
	HMAP_FOR_EACH_WITH_HASH(at_entry, node, hash_string(at_match_string, 0), &(at->at_entrys))
	{
		if(at_entry->last_status == next_state)
		{
			...
			should_drop = execute_fp_actions(ctx, at_entry);
            break;
		}
	}
                    ...
                    ofp_port_t port, uint16_t max_len, bool may_packet_in)
{
	...
	printf("src : %d, dst : %d\n", ctx->xin->flow.nw_src, ctx->xin->flow.nw_dst);
                 ...
                 struct xlate_ctx *ctx)
{
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
		...
		//@sc
		case OFPACT_GOTO_FP: {
			printf("-----sfa get gotofp instruction-----\n");
            struct ofpact_goto_fp *ogt = ofpact_get_GOTO_FP(a);

            //ovs_assert(ctx->table_id < ogt->table_id);
            //xlate_table_action(ctx, ctx->xin->flow.in_port.ofp_port,
            //                   ogt->table_id, true);
			int should_stop = xlate_fp(ctx, ogt->bitmap, true);
