...
ovs_instruction_type_from_ofpact_type(enum ofpact_type type)
{
	...
	//@sc
	case OFPACT_GOTO_FP:
		return OVSINST_OFPIT11_GOTO_FP;
    case OFPACT_METER:
                                   ...
                                   struct ofpbuf *ofpacts)
{
	...
	//@sc
	 if (insts[OVSINST_OFPIT11_GOTO_FP]) {
		 //printf("----sfa get gotofp instruction in ovs-----\n");
        const struct ofp11_instruction_goto_fp *oigt;
        struct ofpact_goto_fp *ogt;

        oigt = instruction_get_OFPIT11_GOTO_FP(
            insts[OVSINST_OFPIT11_GOTO_FP]);
        ogt = ofpact_put_GOTO_FP(ofpacts);
        ogt->bitmap = oigt->bitmap;
    }
               ...
               uint8_t table_id, uint8_t n_tables)
{
	...
	//@sc
	case OFPACT_GOTO_FP: {
        uint8_t bitmap = ofpact_get_GOTO_FP(a)->bitmap;
        if (bitmap == 0) {
//            return OFPERR_OFPBRC_BAD_FPAPP_BITMAP;
        }
        return 0;
    }
                                  ...
                                  enum ofp_version ofp_version)
{
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
		...
		//@sc
		case OVSINST_OFPIT11_GOTO_FP: {
            struct ofp11_instruction_goto_fp *oigt;
            oigt = instruction_put_OFPIT11_GOTO_FP(openflow);
            oigt->bitmap = ofpact_get_GOTO_FP(a)->bitmap;
...
ofpact_format(const struct ofpact *a, struct ds *s)
{
	...
	case OFPACT_GOTO_FP:
        ds_put_format(s, "%s:%"PRIu8,
                      ovs_instruction_name_from_type(
                          OVSINST_OFPIT11_GOTO_FP),
                      ofpact_get_GOTO_FP(a)->bitmap);
        break;
